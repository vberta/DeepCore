...
target_par = np.zeros(shape=(jetNum, trackNum, parNum)) #4=x,y,eta,phi
target_par0 = np.zeros(shape=(jetNum, parNum)) #4=x,y,eta,phi
...
if generate :
    for i in range(jetNum) :
        ...
        print("sample ",i)
        jetMap= np.zeros(shape=(layNum, jetDim,jetDim)) #seq=[]
        trackMap= np.zeros(shape=(trackNum,layNum, jetDim,jetDim)) #n=[]
        trackPar= np.zeros(shape=(trackNum,parNum)) #n=[]
            ...
            #    if x>jetDim/2 or y>jetDim/2 or y<-jetDim/2 or x<-jetDim/2 :
               ...
               for electrons in range(100) :
                       ...
                       if channelx>=-jetDim/2 and channelx < jetDim/2 and channely>=-jetDim/2 and channely < jetDim/2 :
                           jetMap[lay][channelx+jetDim/2][channely+jetDim/2]+=1000*1./100.
                           trackMap[trk][lay][channelx+jetDim/2][channely+jetDim/2] +=1000*1./100.
               if x>=-jetDim/2 and x < jetDim/2 and y>=-jetDim/2 and y < jetDim/2  and lay==0:
                    trackPar[trk] = (x,y,trackDirR[trk],trackDirPhi[trk])
        ...
        trackMapSorted= np.zeros(shape=(trackNum,layNum, jetDim,jetDim)) #n=[]
        trackParSorted= np.zeros(shape=(trackNum,parNum)) #n=[]
        for trk in range(trackNum) :
    ...
    if one_track :
        outfile0 = file("ONETRACK_toy_MC_event_{ev}_layer{llay}_angle{angle}.npz".format(ev=jetNum,llay=layNum, angle=openAngle),"w")
...
if generate==False :
    ...
    for jet in range(jetNum) :
        target_par[jet][0]+=jetDim/2     #FIXME remove jetDim/2: only for RELU activation!!!!!
...
# print ("target shape", target_map0.shape)
# print ("target shape", target_par0.shape)
     ...
     for jet in range(numPrint) :
         ...
         for lay in range(layNum) :
             ...
             if 1 : #maybe if predict can be useful?
                 ...
                 print("target:", target_par[j_eff][0]-jetDim/2,target_par[j_eff][1]-jetDim/2, target_par[j_eff][2], target_par[j_eff][3])
                 print("prediction:", validation_par[j_eff][0]-jetDim/2,validation_par[j_eff][1]-jetDim/2, validation_par[j_eff][2], validation_par[j_eff][3])

                 for i in range(1) : #trackNum
                     d = (lay+1)*layDist*math.tan(target_par[j_eff][2])

                     xoff = (target_par[j_eff][0]-jetDim/2)/(d/(lay+1))-math.sin(target_par[j_eff][3])
                     yoff = (target_par[j_eff][1]-jetDim/2)/(d/(lay+1))-math.cos(target_par[j_eff][3])
                     x = d*(math.sin(target_par[j_eff][3])+xoff) #-jetDim/2
                     y = d*(math.cos(target_par[j_eff][3])+yoff) #-jetDim/2
                    #  x = d*(math.sin(target_par[j_eff][3])+target_par[j_eff][0]-jetDim/2)
                    #  y = d*(math.cos(target_par[j_eff][3])+target_par[j_eff][1]-jetDim/2)
