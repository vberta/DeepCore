...
def calculateEfficiency(tfile, num_name, denom_name, rebin=None,fake=False,drjReb=False,dupl=False):
    ...
    if((fake or dupl)) :
        ...
        den_temp = tfile.Get(den_tot_name)
        den_tot = den_temp.Clone()
    else :
        den_tot = denom.Clone()
        # den_tot_name = denom_name
        # den_tot_name = "DQMData/Run 1/Tracking/Run summary/Track/general_MTVAssociationByChi2/num_simul_drj"
        # den_temp = tfile.Get(den_tot_name)
        # den_tot = den_temp.Clone()
    ...
    # eff_reb = eff.Clone()
    # den_reb = den.Clone()
    # den_tot_reb = den_tot.Clone()
    # print("prima",den_tot_reb.GetSize(),eff_reb.GetSize())
    if(drjReb) :
        eff_reb = eff.Rebin(8,"eff_reb",edg_new)
        den_reb = den.Rebin(8,"den_reb",edg_new)
        den_tot_reb = den_tot.Rebin(8,"den_tot_reb",edg_new)
        eff_reb.SetBinContent(1,eff_reb.GetBinContent(1)+eff_reb.GetBinContent(0))
        den_reb.SetBinContent(1,den_reb.GetBinContent(1)+den_reb.GetBinContent(0))
        den_tot_reb.SetBinContent(1,den_tot_reb.GetBinContent(1)+den_tot_reb.GetBinContent(0))
        if(fake) :
            eff_reb.Add(den_reb,eff_reb,1,-1)
        # print("dopo",den_tot_reb.GetSize(),eff_reb.GetSize())
        eff_reb.Divide(den_tot_reb)
        return eff_reb
    else :
        if(fake) :
            eff.Add(den,eff,1,-1)
        # print("name num=",num_name)
        # print("name den=",den_tot_name)
        # print("num0=",eff.GetBinContent(1), ", den0=",den_tot.GetBinContent(1), ", ratio=",eff.GetBinContent(1)/den_tot.GetBinContent(1))
        eff.Divide(den_tot)
        return eff
...
def plotColoredEff(phase1file, prefix, pileup):
    ...
    # folder_track = "DQMData/Run 1/Tracking/Run summary/Track/cutsReco%sByOriginalAlgoHp_trackingParticleRecoAsssociation/" GOOOD ONE
    folder_track = "DQMData/Run 1/Tracking/Run summary/Track/cutsReco%sByOriginalAlgo_MTVAssociationByChi2/"
    # folder_track = "DQMData/Run 1/Tracking/Run summary/Track/cutsReco%sByOriginalAlgoHp_MTVAssociationByChi2/" #HP!!
    ...
    effbox = PlotTextBox(xmin, 0.85, xmax, 0.92, transparent=True)#lineheight=0.02)#fake effbox_pt.move(dx=0.06+0.7, dy=-0.02+0.58)

    # effbox.addText("t#bar{t} event tracks%s"%putext)
    # effbox.addText("p_{T} > 0.9 GeV, |#eta| < 2.5, ^{}d_{0} < 3.5 cm")
    effbox.addText("QCD 1.8 TeV <#hat p_{T}< 2.4 TeV (no PU)")
    effbox.addText("p_{T}^{jet} > 1 TeV, |#eta^{jet}| < 1.4")
...
#    effbox_pt.move(dx=-0.13, dy=0.6)
    ...
    _common = {
        "xtitle": "Simulated track p_{T} (GeV)",
        "xmin": 1e-1, "xmax": 1e3, "ymin": 0, "ymax": 1,
        "xlog": True,
    ...
    # dr
#     effbox_dr = effbox.clone()
#     effbox_dr.replaceText(1, "|#eta| < 2.5, ^{}d_{0} < 3.5 cm")
#     effbox_dr.move(dx=0.06, dy=-0.02)
# #    effbox_dr.move(dx=-0.13, dy=0.6)
#     plot = PlotStack()
#     for it, leg in zip(iterations, legendLabels):
#         hname = folder_track%it + "effic_vs_dr"
#         h = phase1file.Get(hname)
#         if not h:
#             raise Exception("Did not find %s from %s" % (hname, phase1file.GetName()))
#         h.SetFillColor(iter_colors[it])
...
#     _common = {
#         "xtitle": "Delta R",
#         "xmin": 1e-3, "xmax": 1, "ymin": 0, "ymax": 1,
#         "xlog": True,
#         "legendDx": -0.13, "legendDy": -0.24, "legendDw": legendDw, "legendDh": 0.23,
#         "legendTransparent": True,
#         "customise": lambda: effbox_dr.Draw(),
#         "drawOpt": "HIST",
#     }
#     drawPlot(prefix+"_efficiency_dr_cum", plot, ytitle="Tracking efficiency", **_common)

    #drjet -eff
    effbox_dr = effbox.clone()
    # effbox_dr.replaceText(1, "|#eta| < 2.5, ^{}d_{0} < 3.5 cm")
    # effbox_dr.move(dx=0.06, dy=-0.02)
    # effbox_pt.move(dx=0.06+0.7, dy=-0.02+0.58)
#    effbox_dr.move(dx=-0.13, dy=0.6)
    plot = PlotStack()
    for it, leg in zip(iterations, legendLabels):
        # hname = folder_track%it + "effic_vs_drj"
        # h = phase1file.Get(hname)
        # if not h:
        #     raise Exception("Did not find %s from %s" % (hname, phase1file.GetName()))
        # h.SetFillColor(iter_colors[it])
        ...
        eff = calculateEfficiency(phase1file, num_name, denom_name, rebin=5,drjReb=False)
        eff.SetFillColor(iter_colors[it])
        eff.SetLineColor(ROOT.kBlack)
        plot.add(eff, leg)
    _common = {
        "xtitle": "Delta R(track, jet)",
        # "xmin": 0.0025118869, "xmax": 0.1, "ymin": 0, "ymax": 1,
    ...
    #drjet - fake
    fakebox_drj = effbox.clone()
    # fakebox_drj.replaceText(1, "|#eta| < 2.5, ^{}d_{0} < 3.5 cm")
    # fakebox_drj.move(dx=0.06, dy=-0.02)
    effbox_pt.move(dx=0.06+0.7, dy=-0.02+0.58)
#    fakebox_drj.move(dx=-0.13, dy=0.6)
    plot_fake_drj = PlotStack()
    for it, leg in zip(iterations, legendLabels):
        # hname = folder_track%it + "fakerate_vs_drj"
        # h = phase1file.Get(hname)
        # if not h:
        #     raise Exception("Did not find %s from %s" % (hname, phase1file.GetName()))
        # h.SetFillColor(iter_colors[it])
        # h.SetLineColor(ROOT.kBlack)
        # plot_fake_drj.add(h, leg)
        # h.Rebin(2)
        num_name_fake = folder_track%it + "num_assoc(recoToSim)_drj"
        denom_name_fake = folder_track%it + "num_reco_drj"
        fake_drj = calculateEfficiency(phase1file, num_name_fake, denom_name_fake, rebin=5, fake=True, drjReb=False)
        # fake_drj.Add(fake_drj,fake_drj,1,-2) #fake_drj--> -fake_drj
        # line = ROOT.TF1("line",1,0,1)
        # fake_drj.Add(line)

        fake_drj.SetFillColor(iter_colors[it])
        fake_drj.SetLineColor(ROOT.kBlack)
        plot_fake_drj.add(fake_drj, leg)
    _common = {
        "xtitle": "Delta R(track, jet)",
        # "xmin": 0.0025118869, "xmax": 0.1, "ymin": 0, "ymax": 0.3,
        "xmin": 0.001, "xmax": 0.1, "ymin": 0, "ymax": 0.3,
        "xtitle": "#DeltaR(track,jet)",
        "xlog": True,
        "legendDx": -0.13, "legendDy": -0.24, "legendDw": legendDw, "legendDh": 0.23,
        "legendTransparent": True,
        "customise": lambda: fakebox_drj.Draw(),
        "drawOpt": "HIST",
    }
    drawPlot(prefix+"_fakerate_dr_cum", plot_fake_drj, ytitle="Fake Rate", **_common)


    #duplicate drj
    dupbox_drj = effbox.clone()
    # dupbox_drj.replaceText(1, "|#eta| < 2.5, ^{}d_{0} < 3.5 cm")
    # dupbox_drj.move(dx=0.06, dy=-0.02)
    effbox_pt.move(dx=0.06+0.7, dy=-0.02+0.58)
#    dupbox_drj.move(dx=-0.13, dy=0.6)
    plot_dupl_drj = PlotStack()
    for it, leg in zip(iterations, legendLabels):
        # hname = folder_track%it + "fakerate_vs_drj"
        # h = phase1file.Get(hname)
        # if not h:
        ...
        # h.SetFillColor(iter_colors[it])
        # h.SetLineColor(ROOT.kBlack)
        # plot_dupl_drj.add(h, leg)
        # h.Rebin(2)
        num_name_dupl = folder_track%it + "num_duplicate_drj"
        denom_name_dupl = folder_track%it + "num_reco_drj"
        dupl_drj = calculateEfficiency(phase1file, num_name_dupl, denom_name_dupl, rebin=5, fake=False, drjReb=False, dupl=False)
        # dupl_drj.Add(dupl_drj,dupl_drj,1,-2) #dupl_drj--> -dupl_drj
        # line = ROOT.TF1("line",1,0,1)
        # dupl_drj.Add(line)

        dupl_drj.SetFillColor(iter_colors[it])
        dupl_drj.SetLineColor(ROOT.kBlack)
        plot_dupl_drj.add(dupl_drj, leg)
    _common = {
        "xtitle": "Delta R(track, jet)",
        # "xmin": 0.0025118869, "xmax": 0.1, "ymin": 0, "ymax": 0.3,
        "xmin": 0.001, "xmax": 0.1, "ymin": 0, "ymax": 0.15,
        "xtitle": "#DeltaR(track,jet)",
        "xlog": True,
        "legendDx": -0.13, "legendDy": -0.24, "legendDw": legendDw, "legendDh": 0.23,
        "legendTransparent": True,
        "customise": lambda: dupbox_drj.Draw(),
        "drawOpt": "HIST",
    }
    drawPlot(prefix+"_duplicate_dr_cum", plot_dupl_drj, ytitle="Duplicate Rate", **_common)
